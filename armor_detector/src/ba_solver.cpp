...
namespace fyt::auto_aim
{
                      ...
                      const Eigen::Matrix3d& R_imu_camera) noexcept
    {
        ...
        Eigen::Matrix3d r_yaw = R_imu_armor * R_pitch.inverse().matrix();

        // Compute the initial yaw from rotation matrix
        double initial_armor_yaw;
        auto theta_by_sin = std::asin(-r_yaw(0, 1));
        auto theta_by_cos = std::acos(r_yaw(1, 1));
        ...
        for (size_t i = 0; i < Armor::N_LANDMARKS; i++)
        {
            ...
            EdgeProjection* edge =
                new EdgeProjection(R_camera_imu, R_pitch, t_camera_armor, K_, offset_xy_);
