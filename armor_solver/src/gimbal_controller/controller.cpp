...
void Controller::setup_pitch_solver()
{
    ...
    const Eigen::Vector3d Controller::target_xyz() const {
        return { target_.position.x, target_.position.y, target_.position.z };
    }

    const double Controller::target_distance2d() const {
        return sqrt(
            target_.position.x * target_.position.x
            +
            target_.position.y * target_.position.y
        );
    }

    const double Controller::target_distance3d() const {
        return sqrt(
            target_.position.x * target_.position.x
            +
            target_.position.y * target_.position.y
            +
            target_.position.z * target_.position.z
        );
    }
    ...
    void Controller::transCmd()
    {
        ...
        //double cmd_yaw=plan.yaw, cmd_pitch=plan.pitch;
        ...
        calcYawAndPitch(target_pos, center_yaw, center_pitch);
            ...
            gimbal_cmd.distance = target_distance3d();
            top_ampl_ = 2 * atan2(r, target_distance2d());

            // Compensate angle by angle_offset_map
            auto angle_offset = manual_compensator_->angleHardCorrect(target_distance2d(), target_xyz().z());
    ...
    Controller::Plan Controller::getPlan()
    {

        // 1. Predict fly_time

        double dist = target_xyz().head<2>().norm();
        double bullet_speed = trajectory_compensator_->velocity;
        if (bullet_speed < 10 || bullet_speed > 25) {
            bullet_speed = 22;
        }
        auto bullet_traj_opt = trajectory(bullet_speed, dist, target_xyz().z());
        if (!bullet_traj_opt.has_value()) {
            FYT_ERROR("armor_solver", "Can't solve bullet");

        }
        auto [bullet_pitch, bullet_fly_time] = *bullet_traj_opt;
        kalman_pool_->predict(target_id, bullet_fly_time);
    ...
    Eigen::Matrix<double, 2, 1> Controller::aim()
    {
        double yaw;
        double bullet_speed = trajectory_compensator_->velocity;
        double x = target_.position.x;
        double y = target_.position.y;
        double z = target_.position.z;
        double dist = sqrt(x * x + y * y);

        auto azim = std::atan2(y, x);
        auto bullet_traj = trajectory(bullet_speed, dist, z);
        if (!bullet_traj.has_value()) throw std::runtime_error("Unsolvable bullet trajectory!");

        return { limit_rad(azim + yaw_offset_), -(*bullet_traj).first - pitch_offset_ };
    }
    ...
    Trajectory Controller::get_trajectory(const double yaw0)
    {
        ...
        auto target = kalman_pool_->predict(target_id, -DT * (HALF_HORIZON + 1));

        auto yaw_pitch_last = aim();

        target = kalman_pool_->predict(target_id, DT);  // [0] = -HALF_HORIZON * DT -> [HHALF_HORIZON] = 0
        auto yaw_pitch = aim();

        for (int i = 0; i < HORIZON; i++) {
            target = kalman_pool_->predict(target_id, DT);
            auto yaw_pitch_next = aim();
