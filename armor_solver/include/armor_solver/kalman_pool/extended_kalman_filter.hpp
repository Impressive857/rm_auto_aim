...
#ifndef RM_UTILS_KALMAN_FILTER_HPP_1
#define RM_UTILS_KALMAN_FILTER_HPP_1
...
namespace myself
{
  ...
  public:
    ...
    explicit ExtendedKalmanFilter(PredicFunc* u_f,
                                  MeasureFunc* u_h,
                                  const UpdateQFunc& u_q,
                                  const UpdateRFunc& u_r,
                                  const MatrixXX& p) noexcept
      : f(u_f), h(u_h), update_Q(u_q), update_R(u_r), P_post(p)
    {
      F = MatrixXX::Zero();
      H = MatrixZX::Zero();
    }
    ...
    template<class Measure,int NN_Z>
    MatrixX1 update(const Eigen::Vector<double,NN_Z>& z, Measure* nh,
      std::function<Eigen::Matrix<double,NN_Z,NN_Z>(const Eigen::Vector<double,NN_Z>&)> u_r) noexcept
    {
      ceres::Jet<double, N_X> x_p_jet[N_X];
      for (int i = 0; i < N_X; i++)
      {
      ...
      ceres::Jet<double, N_X> z_p_jet[NN_Z];
      (*nh)(x_p_jet, z_p_jet);

      Eigen::Vector<double,NN_Z> z_pri;
      Eigen::Matrix<double,NN_Z,N_X> H_;
      for (int i = 0; i < NN_Z; i++)
      {
        z_pri[i] = z_p_jet[i].a;
        H_.block(i, 0, 1, N_X) = z_p_jet[i].v.transpose();
      }

      auto R_ = u_r(z);
      auto K_ = P_pri * H_.transpose() * (H_ * P_pri * H_.transpose() + R_).inverse();
      x_post = x_post + K_ * (z - z_pri);
      P_post = (MatrixXX::Identity() - K_ * H_) * P_pri;
      return x_post;
    }
...
#endif  // RM_UTILS_KALMAN_FILTER_HPP_1
