...
public:
  ...
  void PointReprojection(const std::vector<cv::Point3f> &image_points,
                                             const cv::Mat &rvec,
                                             const cv::Mat &tvec,
                                            std::vector<cv::Point2f>& reprojected_points) const noexcept;

  double getBestYaw(const std::vector<cv::Point2f> &image_points,
                    const cv::Mat tvec,
                    const std::string &coord_frame_name) const noexcept;

  double calculateDist(const std::vector<cv::Point2f> &image_points,
                      const cv::Mat tvec,
                      double roll , double pitch , double yaw,
                      const std::string &coord_frame_name) const noexcept;

  cv::Mat rpyToR(double roll,double pitch,double yaw) const noexcept;
